...
{
  mime = {
    {".html", "text/html"},         {".avi", "video/x-msvideo"},    {".bmp", "image/bmp"},
    {".c", "text/plain"},           {".doc", "application/msword"}, {".gif", "image/gif"},
    {".gz", "application/x-gzip"},  {".htm", "text/html"},          {".ico", "image/x-icon"},
    {".jpg", "image/jpeg"},         {".png", "image/png"},          {".txt", "text/plain"}, 
...
{
    connChannel_->setEvent(EPOLLIN | EPOLLET | EPOLLONESHOT);
    connChannel_->setWhatEvent(Channel::kReadEvent);
    loop_->updateChannel(connChannel_); 
}
void HttpData::handleRead()
{
    __uint32_t &events_ = connChannel_->getEvent();
  do 
  {
    bool zero = false;
    int read_num = readn(connFd_, inBuffer_, zero);
    printf("Request: %s\n", inBuffer_.c_str() );
    ...
    if (state_ == STATE_PARSE_URI) 
    {
      ...
      {
        perror("2"); 
        fprintf(stderr, " LOG FD = %d, %s, ******",  connFd_, inBuffer_.c_str());
        inBuffer_.clear();
        error_ = true;
        handleError(connFd_, 400, "Bad Request");
        break;
    ...
    if (state_ == STATE_RECV_BODY) 
    {
      ...
      else 
      {
        ...
        error_ = true;
        handleError(connFd_, 400, "Bad Request: Lack of argument (Content-length)");
        break;
      }
...
void HttpData::handleConn()
{
    ...
    if (!error_ && connectionState_ == H_CONNECTED) 
    {
        if (events_ != 0) 
        {
            ...
            events_ |= EPOLLET;
            loop_->updateChannel(connChannel_, timeout);
...
void HttpData::handleError(int fd, int err_num, std::string short_msg) 
{
    ...
    header_buff += "Server: liuzengh's tiny http server\r\n";
    header_buff += "\r\n";
    // 请求内容
    body_buff += "<html><title>请求出错</title>";
    body_buff += "<body bgcolor=\"ffffff\">";
    body_buff += std::to_string(err_num) + short_msg;
    body_buff += "<hr><em> Server: liuzengh's tiny http server</em>\n</body></html>";
