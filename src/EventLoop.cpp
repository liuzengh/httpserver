    ...
    pwakeupChannel_(new Channel(this, wakeupFd_))
{
    ...
    // 成员函数的bind用法见c++_primer_5th_19.4.3
    ...
    poller_->updateChannel(pwakeupChannel_); // 和pwakeupChannel_->update();的写法实现的功能应该是一样的。
}
...
void EventLoop::updateChannel(std::shared_ptr<Channel> channel, int timeout = 0)
{
    assert(channel->ownerLoop() == this);
    assert(isInLoopThread());
    poller_->updateChannel(channel, timeout = 0);
}
...
void EventLoop::handleRead()
{
    ...
    pwakeupChannel_->setEvent(EPOLLIN | EPOLLET); // why? 
