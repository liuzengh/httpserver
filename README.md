...
webbench是一个简易的服务器压力测试工具，其原理是父进程fork多个子进程充当客户端，父子进程通过管道进行通信。在子进程在退出之前后会向管道里面写入测试结果，父进程读取每个子的测试结果进行汇总。每个子进程的都处在一个死循环中，不断地创建新的连接对服务器发送请求，并统计该次请求的信息，每次轮循环都会检查超时标识，当检查到超时才退出循环。超时的实现思路是在每个子进程用alarm函数创建一个定时器，当超时发送会内核会产生SIGALRM信号，预先设置的信号处理函数会把超时标识置为ture。
